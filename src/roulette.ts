...
export class Roulette extends EventTarget {
  ...
  protected _options: Required<RouletteOptions>;
  ...
  protected createRenderer(): RouletteRenderer {
    return new RouletteRenderer({
      enableKeywordSprites: this._options.enableKeywordSprites,
    });
  }
  ...
  constructor(options: RouletteOptions = {}) {
    super();
    this._options = {
      enableKeywordSprites: options.enableKeywordSprites ?? true,
      showUiOverlays: options.showUiOverlays ?? true,
    };
        ...
        if (this._isRunning && this._winners.length === this._totalMarbleCount) {
          this.dispatchEvent(
            new CustomEvent('goal', {
              detail: {
                finishCount: this._winners.length,
              },
            })
          );
  ...
  private async _init() {
    ...
    this.attachEvent();
    if (this._options.showUiOverlays) {
  ...
  public setMarbleColor(name: string, color: string) {
    const marble = this._marbles.find((candidate) => candidate.name === name);
  ...
  public setMarbles(names: string[]) {
    ...
    // 카메라를 구슬 생성 위치 중앙으로 이동하되 너무 가깝지 않게 여유를 둔다.
    if (totalCount > 0) {
      ...
      const zoom = Math.max(
        1.2,
        Math.min(Math.min(viewW / (spawnWidth + margin * 2), viewH / (spawnHeight + margin * 2)), 2.5)
      );
